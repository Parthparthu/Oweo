...
export const AddGroupExpenseModal: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const totalPaise = parseAmountInput(amountStr)
    ...
    if (splitType === 'equal') {
      calculatedShares = calculateEqualSplit(totalPaise, selectedMemberIds)
    } else if (splitType === 'exact') {
      const sharesPaise: Record<string, number> = {}
      selectedMemberIds.forEach((id) => {
        const amt = parseAmountInput(customAmounts[id] || '0') || 0
        sharesPaise[id] = amt
      })
      const valid = validateCustomSplit(totalPaise, sharesPaise)
      if (!valid.isValid) {
      ...
      const valid = calculatePercentageSplit(totalPaise, pctMap)
      if (!valid.isValid) {
      ...
      participants[id] = {
        ...
        percentage:
          splitType === 'percentage'
            ...
            : (calculatedShares[id] / totalPaise) * 100,
      }
    })
    ...
    try {
      ...
      showToast(`Added ₹${(totalPaise / 100).toFixed(2)} to ${activeGroup.name}`, 'success')
      closeAddGroupExpenseModal()
    } catch (err: any) {
        ...
        <div className="space-y-2 pt-1 bg-muted/40 p-3 xs:p-3.5 rounded-2xl border border-border/70">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto overscroll-contain pr-0.5">
            {activeGroupMembers.map((member) => {
              ...
              const totalPaise = parseAmountInput(amountStr) || 0
              const equalSharePaise = isSelected
                ? Math.floor(totalPaise / selectedMemberIds.length)
                : 0
